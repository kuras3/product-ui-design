...
The checklist is in `references/ai-tells.md`. Read it before finishing any UI. An optional mechanical gate (`scripts/scan-tells.py`) greps generated CSS for the machine-detectable tells (indigo, pure-black shadow, `transition: all`, `scale(0)`, sub-16px inputs).
...
- `scripts/scan-tells.py` — optional mechanical gate (no dependencies; scans CSS/HTML for indigo, pure-black shadow, `transition:all`, `scale(0)`, sub-16px inputs).
